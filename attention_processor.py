...
class XFormersAttnProcessor:
    ...
    def __call__(
        ...
        if encoder_hidden_states == None or index==None or came_posfeat==None:
            text_embeddings_cls = None
        else:
            # print(index)
            text_embeddings_cls = encoder_hidden_states[torch.arange(encoder_hidden_states.shape[0]), index]
        query = attn.to_q(hidden_states,text_embeddings_cls,came_posfeat)
        ...
        key = attn.to_k(encoder_hidden_states,text_embeddings_cls,came_posfeat)
        value = attn.to_v(encoder_hidden_states,text_embeddings_cls,came_posfeat)
        ...
        ########################################################################
        attention_probs = attn.get_attention_scores(query, key, attention_mask)
        attention_probs_qeury = attn.get_attention_scores_for_query(query, key, attention_mask)
        attention_map = None
        if index!=None:
            attention_map = attention_probs_qeury[:, :,index]
            attention_map = attention_map.to(query.dtype)
            attention_map = attn.batch_to_head_dim(attention_map)
            attention_map = torch.mean(attention_map,2)
        hidden_states = torch.bmm(attention_probs, value)
        # hidden_states = xformers.ops.memory_efficient_attention(
        #     query, key, value, attn_bias=attention_mask, op=self.attention_op, scale=attn.scale
        # )
        ...
        # linear proj
        hidden_states = attn.to_out[0](hidden_states,text_embeddings_cls,came_posfeat)
        # dropout
        hidden_states = attn.to_out[1](hidden_states)
