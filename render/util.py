...
def save_image(fn, x : np.ndarray):
    try:
        if os.path.splitext(fn)[1] == ".png":
            imageio.imwrite(fn, np.clip(np.rint(x * 255.0).squeeze(), 0, 255).astype(np.uint8), compress_level=3) # Low compression for faster saving
        else:
...
def load_image_raw(fn) -> np.ndarray:
    #imageio.plugins.freeimage.download()  #--fan modify
    return imageio.imread(fn) ##fan modify(, format='HDR-FI')
