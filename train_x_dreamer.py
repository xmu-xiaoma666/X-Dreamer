    ...
    for it, target in enumerate(dataloader_train):
        ...
        if FLAGS.local_rank == 0:
            ...
            if  save_image:
                ...
                util.save_image(FLAGS.out_dir + '/' + ('mask_%s_%06d.png' % (FLAGS.mode, img_cnt)), result_dict['mask'].detach().cpu().numpy())
        ...
        with torch.cuda.amp.autocast(enabled= True):
            ...
            if if_pretrain == False:
                sds_loss, img_loss, reg_loss, attention_loss = model(target, it, if_normal, if_pretrain= if_pretrain, scene_and_vertices =None)
                if FLAGS.mode =='geometry_modeling':
                    if(it<1000):
                        attention_loss = 0
                else:
        ...
        total_loss = img_loss + reg_loss + sds_loss + attention_loss
        ...
        max_norm = 5.0  
        torch.nn.utils.clip_grad_norm_(guidance.parameters(), max_norm)
        ...
        for param in guidance.parameters():
            param.data = torch.nan_to_num(param.data, nan=0.0, posinf=None, neginf=None)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default='configs_clean3/icecream_geometry_debug.json', help='Config file')
    ...
    parser.add_argument('-o', '--out-dir', type=str, default='results/result_debug/icecream_geometry')
    ...
    if FLAGS.mode == 'geometry_modeling' :   
        ...
        with open(os.path.join(FLAGS.out_dir, "material.pkl"), 'wb') as f:
            pickle.dump(mat['kd_ks_normal'], f)
        # ==============================================================================================
        #  Validate
        # ==============================================================================================
